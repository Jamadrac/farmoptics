...
class ImageAnalyser extends StatefulWidget {
  const ImageAnalyser({super.key});

  @override
  State<ImageAnalyser> createState() => _MyImageAnalyserState();
}

class _MyImageAnalyserState extends State<ImageAnalyser>
  ...
  void initState() {
    ...
    )..addListener(_onTypeAnimation);
  }

  void _onTypeAnimation() {
    if (diseaseName.isNotEmpty && _currentLength < diseaseName.length) {
      setState(() {
        _currentLength = (_typeController.value * diseaseName.length).floor();
        _displayedText = diseaseName.substring(0, _currentLength);
      });
    }
  }
